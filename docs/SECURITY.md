...
- `MEDSENTINEL_IRIS_MODE=memory` keeps all MedSim data in the local dev store.
- `MEDSENTINEL_IRIS_MODE=fhir` uses the live IRIS FHIR repository for interoperability while keeping MedSim domain storage in the local dev store, which is useful when native IRIS global access is not provisioned yet.
